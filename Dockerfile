...
ARG BATS_VERSION
RUN cd /tmp \
    && git clone https://hub.fastgit.org/bats-core/bats-core.git \
    && cd bats-core \
...
RUN mkdir -p /usr/src/criu \
    && curl -fsSL https://hub.fastgit.org/checkpoint-restore/criu/archive/${CRIU_VERSION}.tar.gz | tar -C /usr/src/criu/ -xz --strip-components=1 \
    && cd /usr/src/criu \
    && echo 1 > .gitid \
...
# install umoci
RUN curl -o /usr/local/bin/umoci -fsSL https://hub.fastgit.org/opencontainers/umoci/releases/download/v0.4.5/umoci.amd64 \
